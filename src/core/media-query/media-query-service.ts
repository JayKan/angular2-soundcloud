...
export class MediaQueryService {
  ...
  constructor(@Inject(MEDIA_QUERY_RULES) rules: MediaQueryRule[], zone: NgZone) {
      this.matches$ = Observable
        .combineLatest(...this._getMqlObservables(rules))
        .debounceTime(5)
        .map(this._getResults)
        .enterZone(zone)
        .publishReplay(1)
        .refCount()
  }

  private _getMqlObservables(rules: MediaQueryRule[]): any[] {
  ...
  private _getResults(updates: MediaQueryUpdate[]): MediaQueryResults {
