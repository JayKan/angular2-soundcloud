...
export function getPlayer(): Selector<AppState, PlayerState> {
  return state$ => state$
    .map((state: AppState) => state.player)
...
export function getPlayerTrack(): Selector<AppState, Track> {
  return state$ => state$
    ...
    .distinctUntilChanged()
    .withLatestFrom(state$.let(getTracks()), (trackId, tracks) => tracks.get(trackId))
    .filter(track => !!track)
    .distinctUntilChanged();
}
...
export function getPlayerTracklist(): Selector<AppState, Tracklist> {
  return state$ => state$
    .map((state: AppState) => state.player.tracklistId)
    .combineLatest(state$.let(getTracklists()), (tracklistId, tracklists) => tracklists.get(tracklistId))
...
export function getTimes(): Selector<AppState, TimesState> {
  return state$ => state$
    .map((state: AppState) => state.times)
    .distinctUntilChanged();
}